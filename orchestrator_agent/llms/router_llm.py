...
from orchestrator_agent.config import (
    ROUTER_MODEL,
...
class DynamicRouterLLM(BaseChatModel):
    ...
    def _run_litellm(
        ...
        **kwargs: Any
        ...
        logger.info(f"Executing {'async ' if is_async else ''}with LiteLLM. Model: {primary_model}, Fallbacks: {fallback_models}")
        print(f"Executing {'async ' if is_async else ''}with LiteLLM. Model: {primary_model}, Fallbacks: {fallback_models}", flush=True)
    ...
    def route_query(self, messages: List[BaseMessage]) -> tuple[str, str, str]:
        ...
        try:
            ...
            logger.info(f"Invoking router model classification. Primary: {primary_router}, Fallbacks: {fallback_routers}")
            print(f"Invoking router model classification. Primary: {primary_router}, Fallbacks: {fallback_routers}", flush=True)
            ...
            logger.info(f"[Router Decision] Provider: {provider}, Model: {model}, Type: {task_type}, Reason: {decision.reason}")
            print(f"[Router Choice] Model that classified: {response.model}", flush=True)
            print(f"[Router Decision] Provider: {provider}, Model: {model}, Type: {task_type}, Reason: {decision.reason}", flush=True)
            
            return provider, model, task_type
        except Exception as e:
            fallback_provider, fallback_model = ROUTER_FALLBACK_CHAIN[0]
            logger.warning(f"Routing classification failed: {e}. Falling back to default: {fallback_provider}/{fallback_model}")
            print(f"Routing classification failed: {e}. Falling back to default: {fallback_provider}/{fallback_model}", flush=True)
