...
class LiteLLMChat(BaseChatModel):
    ...
    def _run_litellm(
        ...
        logger.info(
            f"LiteLLMChat executing {'async ' if is_async else ''}with LiteLLM. Model: {primary_model}, Fallbacks: {fallback_models}"
        )
        print(
            f"LiteLLMChat executing {'async ' if is_async else ''}with LiteLLM. Model: {primary_model}, Fallbacks: {fallback_models}",
            flush=True,
    ...
    def bind_tools(self, tools: Sequence[Any], **kwargs: Any) -> Any:
        ...
        return LiteLLMChat(user_controls_input=self.user_controls_input, tools=tools, **kwargs)
